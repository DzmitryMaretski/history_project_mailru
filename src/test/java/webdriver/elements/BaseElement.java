...
public abstract class BaseElement extends BaseEntity {
	...
	private static final int TIMEOUT_EXPLICIT = 30;

	private static final int TIMEOUTPAGE = 0;
	/**
	...
	public static List<WebElement> findElements(By element) {
		...
		list = browser.getDriver().findElements(element);
		return list;
	}
	/**
	...
	public void clickAndWait(final By param) {
		waitForIsElementPresent();
		info(getLoc("loc.clicking"));
		browser.getDriver().getMouse().mouseMove(element.getCoordinates());
		if (browser.getDriver() instanceof JavascriptExecutor) {
		...
		element.click();
		WebElement someElement = (new WebDriverWait(browser.getDriver(), Long.valueOf(TIMEOUT_EXPLICIT)))
